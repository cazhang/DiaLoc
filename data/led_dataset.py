...
class LEDDataset(Dataset):
    ...
    def gather_all_floors(self, index, location, mesh_conversion):
        ...
        for enum, f in enumerate(floors):
            ...
            if self.config['light_aug']: # original LED
                ...
                # call transform
                img, tar = self.my_transform(img, tar, [self.RndCropResizeOp, self.RndRotateOp])
                all_maps[enum,...] = img
                all_tars[enum,...] = tar 

            all_conversions[enum, :] = self.mesh2meters[sn][f]["threeMeterRadius"] / 3.0
        
        return all_maps, all_tars, all_conversions
    ...
    def __getitem__(self, index):
        ...
        if self.config["model_ver"]==1: # for blip, use special [LOC]
            text += '[LOC]'
        ...
        if self.config["model_ver"]==1: # for bilp, replace CLS with DEC
            tokens[0] = self.tokenizer.bos_token_id
    
        attention_mask = tokenized_data.attention_mask[0]
        caption_len = attention_mask.sum()

        decode_caption = self.tokenizer.decode(tokens, skip_special_tokens=True)
        self.font = self.font or ImageFont.load_default()
        cap_img = create_image_of_text(decode_caption.encode('ascii', 'ignore'), width=224, nrows=10, font=self.font)

        if tokens[-1] not in [self.tokenizer.sep_token_id, self.tokenizer.pad_token_id]:
            tokens[-1] = self.loc_token_idx

        maps, target, conversions = self.gather_all_floors(index, location, mesh_conversion)
...
class Loader:
    def __init__(self, args, config):
        self.mesh2meters = json.load(open(os.path.join(args.image_dir, "pix2meshDistance.json")))
    ...
    def load_image_paths(self, data, mode):
        ...
        for data_obj in data:
            ...
            #dialogs.append(data_obj["dialogArray"])
            level = 0
            if mode != "test":
