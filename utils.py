...
class MetricLogger(object):
    ...
    def get_avg(self):
        avg_dict = {}
        for name, meter in self.meters.items():
            avg_dict[name] = meter.global_avg
        return avg_dict
...
def confidence_from_pixels(args, preds, targets, mode):
    "Caculate confidences of predictions at the GT pixel location "
    # preds: B x h x w
    # targt: B x h x w 
    assert preds.shape == targets.shape 
    # get B locations
    B = targets.size(0) 
    preds = preds.exp().view(B,-1)
    #print(preds.sum(1))
    gt_inds = targets.view(B,-1).max(dim=1)[1]
    pred_conf = preds.gather(1, gt_inds.unsqueeze(1)).view(-1).numpy()
    return pred_conf 


def snap_to_euclid(tv, node2pix, sn, pred_coord, conversion, level):
    gt_coord = node2pix[sn][tv][0][1], node2pix[sn][tv][0][0]
    dist = np.sqrt(
...
def euclid_distance_from_pixels(args, max_floors, preds, mesh_conversions, info_elem, mode, inter_le=False):
    ...
    if inter_le:
        dialogs, levels, scan_names, episode_ids, true_viewpoints = [],[],[],[],[]
        for entry in info_elem:
            dialogs.append(entry[0])
            levels.append(entry[1])
            scan_names.append(entry[2])
            episode_ids.append(entry[3])
            true_viewpoints.append(entry[4])
    else:
        dialogs, levels, scan_names, episode_ids, true_viewpoints = info_elem
    for pred, conversion, sn, tv, id, level in zip(
        preds, mesh_conversions, scan_names, true_viewpoints, episode_ids, levels
    ):
        ...
        episode_predictions.append([id, dist])
    return distances, episode_predictions
    
def distance_from_pixels(args, max_floors, preds, mesh_conversions, info_elem, mode, inter_le=False):
    """Calculate distances between model predictions and targets within a batch.
    Takes the propablity map over the pixels and returns the geodesic distance"""
    node2pix = json.load(open(args.image_dir + "allScans_Node2pix.json"))
    ...
    if inter_le:
        path, levels, scan_names, episode_ids, true_viewpoints = [],[],[],[],[]
        for entry in info_elem:
            path.append(entry[0])
            levels.append(entry[1])
            scan_names.append(entry[2])
    ...
    for pred, conversion, sn, tv, id, level in zip(
        ...
        if max_floors>1:
            pred_viewpoint = snap_to_grid(geodistance_nodes[sn], node2pix, sn,
                [pred_coord[1].item(), pred_coord[2].item()],
                convers,
                pred_coord[0].item(),
            )
